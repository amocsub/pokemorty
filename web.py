...
def init_db():
    ...
    conn.execute(
        'CREATE TABLE IF NOT EXISTS pokemons (pokemon_id INTEGER PRIMARY KEY, pokemon_name TEXT, pokemon_image TEXT)')
    ...
    conn.executemany(
        "INSERT OR IGNORE INTO pokemons (pokemon_id, pokemon_name, pokemon_image) values (?, ?, ?)", pokemons)
    ...
    def _wrapper(*args, **kwargs):
        try:
            ...
            if master_id_cookie:
                conn = sqlite3.connect('database.db')
                result = conn.execute(
                    "SELECT pokemon_id FROM pokemons_discovered WHERE master_id = %s" % master_id_cookie)
...
def authenticate():
    try:
        ...
        if valid_user:
            ...
            response = redirect("/login")
            response.set_data(
                "db_raw_data: " + str(base64.b64encode(str(result_raw).encode('ascii'))))
            response.set_cookie(
                "authenticated", "VGhpcyBpcyBub3QgbW9yZSB0aGFuIGEgZGlzdHJhY3Rpb24sIGJ1dCBpdCB3YXMgZnVuIHRvIHNlZSB5b3Ugd2FzdGluZyB5b3VyIHRpbWUhISEKCkhBSEE=")
            return response
    except:
        abort(404)
...
def pokemon(pokemon_id: int = 0):
    try:
        conn = sqlite3.connect('database.db')
        result = conn.execute(
            "SELECT pokemon_image, pokemon_name FROM pokemons WHERE pokemon_id = %s" % pokemon_id)  # IDOR
...
def pokedex():
    try:
        ...
        response = make_response(render_template(
            "pokedex.html", pokemon_master=username, pokemon_list=pokemon_list))
        return response
    except:
        abort(404)
